...
(define-data-var pool {symbol: (string-ascii 32), token0: principal, token1: principal, lp-token: principal, fees: principal,
  ...
  {symbol: "wSTX-sBTC", token0: .wstx, token1: .sbtc-token, lp-token: .wstx, fees: .univ2-fees-v1_0_0-0070,
...
(define-public (mock-seed (r0 uint) (r1 uint))
  (begin
    ...
    (try! (contract-call? .sbtc-token transfer r1 tx-sender current-contract none))
...
(define-public (swap (token-in principal) (token-out principal) (fees principal) (amt-in uint) (amt-out-desired uint))
  (let ((pool_ (var-get pool)) (user tx-sender)
    ...
    (try! (contract-call? .sbtc-token transfer amt-in user current-contract none))
    (try! (as-contract? ((with-stx amt-out)) (try! (stx-transfer? amt-out tx-sender user))))
    (if (> amt-fee-protocol u0)
      (begin
        (try! (as-contract? ((with-ft .sbtc-token "sbtc-token" amt-fee-protocol))
          (try! (contract-call? .sbtc-token transfer amt-fee-protocol tx-sender .univ2-fees-v1_0_0-0070 none))))
