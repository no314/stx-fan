...
(define-public (mock-set-bin (id uint) (x uint) (y uint))
  (begin
    ...
    (if (> y u0) (try! (contract-call? .sbtc-token transfer y tx-sender current-contract none)) true)
    (ok true)))
...
(define-read-only (get-pool-for-swap (is-x-for-y bool))
  (ok {
    ...
    x-token: .token-stx-v-1-2, y-token: .sbtc-token,
...
(define-public (pool-transfer (token principal) (amount uint) (recipient principal))
  (begin
    ...
    (if (is-eq token .token-stx-v-1-2)
      ...
      (begin
        (asserts! (is-eq token .sbtc-token) ERR_INVALID_TOKEN)
        (as-contract? ((with-ft .sbtc-token "sbtc-token" amount))
