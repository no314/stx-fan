...
import { boot, check, isOk, okVal, MANAGER_ID } from "./harness.mjs";
const t = await boot();
...
// STX electors: settle-many + convert + payout-many at 50
{
  ...
  for (const s of batch) call("signer-manager-stx-payout", "set-stx-payout", [Cl.bool(true)], s);
...
// Jing route: deposit, partial settlement, reconcile (order stays open), cancel
{
  cycle++;
  ...
  m("claim-rewards", [Cl.list([]), Cl.uint(cycle)], w5);
  const entries = batch.map((s) => Cl.tuple({ staker: P(s), "reward-cycle": Cl.uint(cycle), "bond-index": Cl.none() }));
  m("settle-many", [Cl.list(entries)], w5);
  const rem = BigInt(okVal(mro("get-pending-conversion", []))["convert-epoch-remaining"].value);
  const PRICE = 31_390_486_455_069n;
  const d = m("jing-deposit", [Cl.uint(rem), Cl.uint(1)], w5); check("jing-deposit", isOk(d), Cl.prettyPrint(d.result)); costRow("jing-deposit", 1, d);
  const JING = "sbtc-stx-0-jing-v2";
  call(JING, "mock-buy", [Cl.uint(1_000_000_000n)]); // 1,000 STX: partial
  call(JING, "close-deposits", []);
  call(JING, "mock-settle", [Cl.uint(PRICE)]);
  const r1 = m("jing-reconcile", [], w5); check("jing-reconcile partial", isOk(r1) && Cl.prettyPrint(r1.result).includes("order-open: true"), Cl.prettyPrint(r1.result)); costRow("jing-reconcile (partial fill, order stays open)", 1, r1);
...
console.log("manager source bytes (sim):", simnet.getContractSource("signer-manager-stx-payout").length);
